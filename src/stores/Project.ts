...
interface ProjectSlice extends ProjectState {
  patchProject: (
    key: keyof Exclude<ProjectState, "tasks">,
    val: unknown
  ) => void;
...
const createProjectSlice: StateCreator<ProjectSlice, [], [], ProjectSlice> = (
  set
) => ({
  ...initialState,
  ...
  patchTask: (task) =>
    set((state) => {
      const newTasks = state.tasks.map((t) => {
        return t.detail_id === task.detail_id ? task : t;
      });
  ...
  removeTask: (task) =>
    set((state) => {
      ...
      const newComments = state.comments.filter(
        (c) => !(c.obs_type === "task" && c.obs_id === task.id)
      );
